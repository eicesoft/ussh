...
- 本地终端：点击标签栏最后一个标签右侧的 `+` 按钮直接打开本机 shell，与 SSH 会话共用终端体验（尺寸自适应、日志记录、广播输入）。
...
![uSSH 工作区与连接树](frontend/ussh-workspace.png)

![uSSH 工具面板](frontend/ussh-utility-panel.png)

工作区区域位于左侧栏顶部。默认包含“默认”工作区，可通过 `+` 创建新的工作区；每个工作区拥有独立的连接标签页。切换工作区只切换当前可见标签页，不会主动断开其他工作区中的 SSH 会话。
...
1. 在左侧栏的“工作区”标题右侧点击 `+`。
2. 输入工作区名称并创建，应用会自动切换到新工作区。
3. 在当前工作区打开或新建连接，连接标签页会归属于当前工作区。
4. 切换到其他工作区时，原工作区的标签页和内存中的 SSH 会话会继续保留；切换回来即可直接恢复。

应用退出时只持久化已保存连接对应的标签页。下次启动会恢复上次使用的工作区，并重新打开这些工作区中的已保存连接标签页；临时连接不会自动恢复。
...
- WebView 本地存储：
  - `ussh-open-tabs`：工作区列表、上次打开的工作区以及各工作区中已保存连接的标签页。
  - `ussh-settings`：主题、界面密度、终端、GPU、窗口材质和 AI 配置。
  - `ussh-quick-commands`：快捷命令。
  - `ussh-ai-agent-sessions`：AI 智能体会话历史。
